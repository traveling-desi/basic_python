...
##     	for a in here:
##     		for b in here:
## 			newState = ( (here - frozenset([a, b])), (there | frozenset([a, b])), 1)
## 			retDict[newState] = ( set([a, b]), '->')
##     else:
##         for a in there:
##                 for b in there:
##                         newState = ( (here | frozenset([a, b])), (there - frozenset([a, b])), 0)
##                         retDict[newState] = ( set([a, b]), '<-')
##     return retDict
    ...
    if not light:
    	...
    	return(dict( ( ( (here | frozenset([a, b])), (there - frozenset([a, b])), 0), (set([a, b]), '<-') )
					for a in there
					for b in there))
