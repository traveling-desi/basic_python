...
def double_out(total):
    ...
    validSingleDartScore = []

    for j in range(3):
    	validSingleDartScore.append([])
    	for i in range(20,0,-1):
		validSingleDartScore[j].append(i*(j+1))
	
    validSingleDartScore[0].append(0)
    validSingleDartScore[0].append(25)
    validSingleDartScore[1].append(50)
 
    validScore = [(d1,d2, total-(d1+d2)) for j in range(3) for d1 in validSingleDartScore[j] for i in range(3) for d2 in validSingleDartScore[i] if total-(d1+d2) in validSingleDartScore[1]]
	...
	for i in range(len(d)):
		...
		if dart:
			for j in range(3):
				if dart in validSingleDartScore[j]:
					dartValue = dart/(j+1)
					if dartValue == 25 or dartValue == 50:
						dartValue = 'B'
					dartList.append(lookup[j+1] + str(dartValue))
	return dartList
    ...
    validScore = nameScore
    validScore = sorted(validScore, key=len)
    if len(validScore):
    	return validScore[0]
