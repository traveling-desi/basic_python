...
def do(action, state):
    ...
    foxes, hens = cards.count('F'), cards.count('H')
    if card == 'F':
	foxes -= 1
	retState =  (score, 0, 'F'*foxes + 'H'*hens)
    else:
	hens -= 1
	retState =  (score, yard+1, 'F'*foxes + 'H'*hens)

    if action == 'gather':
	retState =  (score+yard, 0, 'F'*foxes + 'H'*hens)
