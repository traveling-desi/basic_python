    ...
    def successors(*args):
    	...
    	for i in range(len(stateT)):
            ...
            if state[i] < capacities[i]:
                    ...
                    retDict[tuple(state)] = ('fill', i)
            state = list(stateT)
            if state[i] > 0:
                    state[i] = 0
                    retDict[tuple(state)] = ('empty', i)
            for j in range(len(stateT)):
                    if i != j:
                        state = list(stateT)
                        while state[i] < capacities[i] and state[j] > 0:
                            state[i] += 1
                            state[j] -= 1
                        retDict[tuple(state)] = ('pour', j, i)
	#print retDict
    	return retDict

    return shortest_path_search(start, successors, eval(is_goal))
